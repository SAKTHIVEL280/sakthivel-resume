...
const Hyperspeed = forwardRef<any, HyperspeedProps>(({ effectOptions = {}, className }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    speedUp: () => {
      if (appInstance.current) {
        appInstance.current.onMouseDown();
      }
    },
    slowDown: () => {
      if (appInstance.current) {
        appInstance.current.onMouseUp();
      }
    }
  }));
  ...
  return (
    <div id="lights" ref={hyperspeed} className={className}></div>
  );
});
